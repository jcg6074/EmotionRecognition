...
class Stem(nn.Module):
    ...
    def __init__(self, input_dim, dim):
        super().__init__()
        self.norm1 = LayerNorm(dim, 14)
        self.norm2 = LayerNorm(dim, 14)
        self.norm3 = LayerNorm(dim, 14)
...
class decoder(nn.Module):
    def __init__(self):
        super().__init__()
        self.msa = MSA(64)
        self.cnnt_layer1 = nn.Conv2d(197,394,kernel_size=(1,1),stride=1,padding=0)
        self.cnn_layer = nn.Conv1d(64,5,kernel_size=1,stride=1,padding=0)
        self.act = nn.GELU()
        self.mask_token = nn.Parameter(torch.zeros(1, 1, 64*14))
...
class FastSlow(nn.Module):
    ...
    def forward(self, x, p, m):
        ...
        cls_token = cls_token.unsqueeze(dim=-1).repeat(1,1,394,5)
        #x_ = torch.cat((cls_token,x_),dim=2)
