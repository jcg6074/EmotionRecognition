...
class SeedDatasetV3(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        cls_token = cls_token.unsqueeze(dim=-1).repeat(1,265,5)
        origin_input = origin_input.transpose(0,1)
        origin_input = torch.cat((cls_token,origin_input),dim=0)
        origin_input = origin_input.transpose(0,1)
        af_mask = torch.zeros(265,65,5)
        #af_mask = torch.ones(265,65,5)
        target_mask = torch.zeros(265,62,5)
