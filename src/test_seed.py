...
class decoder(nn.Module):
    def __init__(self):
        super().__init__()
        self.dense = nn.Linear(3,34*8)
        self.cnnt_layer1 = nn.ConvTranspose2d(1,64,kernel_size=(1,2),stride=2,padding=0)
        self.cnnt_layer2 = nn.ConvTranspose2d(64,64,kernel_size=(1,1),stride=2,padding=0)
        self.cnnt_layer3 = nn.ConvTranspose2d(64,5,kernel_size=(1,2),stride=2,padding=0)
        self.act = nn.GELU()
    def forward(self,y):
        de = self.dense(y)
        B, T= de.shape
        de = de.reshape(B,1,34,8)
        de = self.cnnt_layer1(de)
        de = self.cnnt_layer2(de)
        de = self.cnnt_layer3(de)
        return de
